...
#pragma omp section
                                        ...
                                        std::vector<Object> &objects)
    {
        ...
        {
            float *pdata = (float *)feat + i * 16;

            float box_score = pdata[4];
            float face_score = pdata[15];
            if (box_score > 0.25 || face_score > 0.25)
            {
                // LOG_INFO("box_score %ld:%f", i, box_score);
            }
            else
            {
                // LOG_INFO("box_score %ld:%f", i, box_score);
            ...
            obc.rect = cv::Rect_<float>(x, y, width, height);
            ...
            for (int k = 5; k < 15; k += 2)
            {
                ...
                float x = pdata[k];
                float y = pdata[k + 1];
                obc.landmark[ind] = cv::Point2f(x, y);
            }
    ...
    void generate_proposals_yolov8(int stride, const float *dfl_feat, const float *cls_feat,
                                   const float *cls_idx, float prob_threshold, std::vector<Object> &objects,
                                   int letterbox_cols,
                                   int letterbox_rows, int cls_num)
    {
        int feat_w = letterbox_cols / stride;
        int feat_h = letterbox_rows / stride;
        int reg_max = 16;
    ...
    void generate_proposals_yolov8_seg(int stride, const float *dfl_feat, const float *cls_feat, const float *cls_idx, float prob_threshold, std::vector<Object> &objects,
    ...
    void generate_proposals_yolov8_seg_native(int stride, const float *feat, const float *feat_seg, float prob_threshold, std::vector<Object> &objects,
                                              int letterbox_cols, int letterbox_rows, int cls_num, int mask_proto_dim)
    {
        int feat_w = letterbox_cols / stride;
        int feat_h = letterbox_rows / stride;
        int reg_max = 16;
    ...
    void generate_proposals_yolov8_pose_native(int stride, const float *feat,
                                               ...
                                               float prob_threshold, std::vector<Object> &objects,
                                               int letterbox_cols, int letterbox_rows,
                                               const int num_point, int cls_num)
    {
        int feat_w = letterbox_cols / stride;
        int feat_h = letterbox_rows / stride;
        int reg_max = 16;
        ...
        // int channels = 65;
        // std::vector<float> feat_(channels * feat_w * feat_h);
        ...
        //         {
        //             int dstIdx = h * feat_w * channels + w * channels + c;
        //             int srcIdx = c * feat_h * feat_w + h * feat_w + w;
        //             feat_[dstIdx] = feat[srcIdx];
        //         }
        //     }
        // }

        // channels = 51;
        // std::vector<float> feat_kps_(channels * feat_w * feat_h);
        // for (int h = 0; h < feat_h; ++h)
        // {
        //     for (int w = 0; w < feat_w; ++w)
        //     {
        ...
        //             int dstIdx = h * feat_w * channels + w * channels + c;
        //             int srcIdx = c * feat_h * feat_w + h * feat_w + w;
        //             feat_kps_[dstIdx] = feat_kps[srcIdx];
        //         }
        //     }
        // }

        // auto feat_ptr = feat_.data();
        // auto feat_kps_ptr = feat_kps_.data();

        auto feat_ptr = feat;
        auto feat_kps_ptr = feat_kps;
        ...
        for (int h = 0; h <= feat_h - 1; h++)
        {
            for (int w = 0; w <= feat_w - 1; w++)
            {
                ...
                if (box_prob > prob_threshold)
                {
                    ...
                    for (int k = 0; k < num_point; k++)
                    {
                        float kps_x = (feat_kps_ptr[k * 3] * 2.f + w) * stride;
                        float kps_y = (feat_kps_ptr[k * 3 + 1] * 2.f + h) * stride;
                        float kps_s = sigmoid(feat_kps_ptr[k * 3 + 2]);
                        obj.kps_feat.push_back(kps_x);
                        obj.kps_feat.push_back(kps_y);
                        obj.kps_feat.push_back(kps_s);
                    }
        ...
        int src_rows, int src_cols)
    {
        ...
        float ratio_x = (float)src_cols / (float)letterbox_cols;
        float ratio_y = (float)src_rows / (float)letterbox_rows;
                      ...
                      int src_rows, int src_cols)
    {
        ...
        // {
        //     printf("nms: %d %f,%f %f,%f \n", proposals[i].label, proposals[i].rect.x, proposals[i].rect.y, proposals[i].rect.width, proposals[i].rect.height);
        // }
                                       ...
                                       int src_rows, int src_cols)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                objects[i].kps_feat[j * 3] = std::max(
                    std::min((objects[i].kps_feat[j * 3] - 0) * ratio_x, (float)(src_cols - 1)), 0.f);
                objects[i].kps_feat[j * 3 + 1] = std::max(
                    std::min((objects[i].kps_feat[j * 3 + 1] - 0) * ratio_y, (float)(src_rows - 1)), 0.f);
            }
        }
    }
