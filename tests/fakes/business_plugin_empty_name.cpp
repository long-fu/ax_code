...
class EmptyNamePlugin final : public BusinessPlugin
{
public:
    ...
    {
        std::ofstream(BUSINESS_EMPTY_NAME_LIFECYCLE, std::ios::app) << "init\n";
        return 0;
    }
    ...
    {
        std::ofstream(BUSINESS_EMPTY_NAME_LIFECYCLE, std::ios::app)
            << "shutdown\n";
    }
};
...
{
    return new EmptyNamePlugin();
}

extern "C" __attribute__((visibility("default")))
void DestroyPlugin(BusinessPlugin* plugin)
{
    std::ofstream(BUSINESS_EMPTY_NAME_LIFECYCLE, std::ios::app) << "destroy\n";
    delete plugin;
}

__attribute__((destructor)) static void OnLibraryUnload()
{
    std::ofstream(BUSINESS_EMPTY_NAME_LIFECYCLE, std::ios::app) << "dlclose\n";
}
